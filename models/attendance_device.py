...
class AttendanceDevice(models.Model):
    ...
    def _unlink_except_active_device(self):
        for device in self:
            if device.active:
    ...
    def _update_device_info_from_request(self):
        ...
        if 'DeviceType' in request_params:
            # Map the incoming device type to one of the allowed values
            device_type_value = request_params.get('DeviceType', '').lower()
            if device_type_value == 'zk' or 'zkteco' in device_type_value:
                updates['device_type'] = 'zk'
            elif device_type_value == 'essl' or 'essl' in device_type_value:
                updates['device_type'] = 'essl'
            else:
    ...
    def _process_options_data(self, data):
        """
        Process options data from ESSL device
        """

        
        # Parse the options data
        # Format: DeviceName,MAC,TransactionCount,MaxAttLogCount,UserCount,MaxUserCount,MaxUserPhotoCount,Platform,OEMVendor,FWVersion,PushVersion,IPAddress,Language
        for line in data.split('\n'):
            ...
            try:
                ...
                if key == 'DeviceName':
                    self.device_name_info = value
                elif key == 'MAC':
                    self.mac = value
                elif key == 'TransactionCount':
                    self.transaction_count = int(value)
                elif key == 'MaxAttLogCount':
                    self.max_att_log_count = int(value)
                elif key == 'UserCount':
                    self.user_count = int(value)
                elif key == 'MaxUserCount':
                    self.max_user_count = int(value)
                elif key == 'MaxUserPhotoCount':
                    self.max_user_photo_count = int(value)
                elif key == 'Platform':
                    self.platform = value
                elif key == 'OEMVendor':
                    self.oem_vendor = value
                elif key == 'FWVersion':
                    self.fw_version = value
                elif key == 'PushVersion':
                    self.push_version = value
                elif key == 'IPAddress':
                    self.ip_address = value
                elif key == 'Language':
                    self.language = int(value)
                else:
